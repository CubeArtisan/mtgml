...
#include "mtgml/threaded_queue.h"
...
struct DraftbotGenerator {
  using ProcessedValue = std::shared_ptr<std::vector<Pick>>;
...
private:
  ...
  pcg32 main_rng;
  ThreadedQueue<std::size_t, 1 << 24, 1> chunk_queue{};
  ThreadedQueue<std::size_t, 1 << 24, 1>::Writer chunk_writer{chunk_queue.get_writer()};
  ThreadedQueue<ProcessedValue, SHUFFLE_BUFFER_SIZE / 16, 1> processed_queue{};
  BatchPool<Pick, SHUFFLE_BUFFER_SIZE / 16> batch_pool;
...
public:
  DraftbotGenerator(std::string filename, std::size_t batch_size, std::size_t seed)
      : mmap(filename), batch_size{batch_size}, seed{seed}, num_picks{mmap.size() / sizeof(Pick)},
        main_rng{seed, 1}, batch_pool{batch_size}
  { }
  ...
  void queue_new_epoch() & {
    ...
    for (std::size_t i=0; i < (indices.size() / read_ratio) * read_ratio; i++) {
      chunk_writer.write(indices[i]);
    }
  }

  Result next() & {
    auto batched = new std::pair<std::vector<Pick>*, BatchPool<Pick, SHUFFLE_BUFFER_SIZE / 16>*>{{}, &batch_pool};
    {
      py::gil_scoped_release gil_release;
      ProcessedValue processed_value = processed_queue.pop();
      if (!processed_value) {
        std::cout << "Waiting on a draftbot sample." << std::endl;
        queue_new_epoch();
        processed_value = processed_queue.pop_blocking()
      }
      batched->first = processed_value.get();
    }
    py::capsule free_when_done{batched,  [](void* ptr) {
      auto pair = reinterpret_cast<std::pair<std::vector<Pick>*, BatchPool<Pick, SHUFFLE_BUFFER_SIZE / 16>*>*>(ptr);
      pair->second->release_batch(pair->first);
    }};
    Pick& first_pick = batched->first->front();
    return {
      ...
      for (std::size_t read_idx : read_indices) {
        if (shuffle_buffer.size() < SHUFFLE_BUFFER_SIZE) {
          ...
          if (current_batch_idx >= batch_size) {
            if (processed_queue.size_approx() >= 2 * SHUFFLE_BUFFER_SIZE / batch_size) {
              while (processed_queue.size_approx() >= SHUFFLE_BUFFER_SIZE / batch_size && !st.stop_requested()) {
