...
sample_no_replacement(const std::size_t count, std::valarray<float> weights,
                      const std::vector<std::size_t> &actual_indices,
                      pcg32 &rng) {
  weights /= std::max(weights.sum(), 1.f);
  std::vector<float> cum_sum(weights.size());
  std::inclusive_scan(std::begin(weights), std::end(weights),
                      std::begin(cum_sum));
  ...
  for (std::size_t i = 0; i < count; i++)
    reservoir.emplace(exp_dist(rng) / weights[i], i);
  for (auto iter = std::begin(weights) + count; iter != std::end(weights);
       ++iter) {
    ...
    const auto cum_sum_iter =
        std::upper_bound(std::begin(cum_sum) + cur_index, std::end(cum_sum),
    ...
    if (cum_sum_iter != std::end(cum_sum)) {
      ...
      const float e_2 = std::log(
          std::uniform_real_distribution<float>(std::exp(nt_w), 1)(rng));
...
struct CubeGenerator {
  using Result =
      std::tuple<py::array_t<std::uint16_t>, py::array_t<std::uint16_t>>;
...
public:
  CubeGenerator(std::string cube_filename, std::size_t num_cards,
                std::size_t batch_size, std::size_t seed, double noise,
                double noise_std)
      : num_cards{num_cards},
        cube_mmap(cube_filename), num_cubes{cube_mmap.size() /
                                            sizeof(CubeCards)},
        batch_size{batch_size}, length{num_cubes / batch_size},
        initial_seed{seed}, neg_sampler(0.0, num_cards),
        true_indices(num_cards, 0), replacing_neg_sampler(0.0, num_cards),
        noise_dist(noise, noise_std), main_rng(initial_seed, 1),
        task_producer{task_queue}, processed_consumer{processed_queue} {
    py::gil_scoped_release release;
    auto cube_file_start =
        reinterpret_cast<const CubeCards *>(cube_mmap.data());
    ...
    for (std::size_t i = 0; i < num_cubes; i++) {
      ...
      for (const auto card_idx : row) {
        if (card_idx == 0 || card_idx > num_cards)
          break;
    ...
    std::ranges::sort(sortable_sampler, [](const auto &p1, const auto &p2) {
      return p1.first > p2.first;
    });
    std::ranges::transform(sortable_sampler, std::begin(neg_sampler),
                           [](const auto &p) { return p.first; });
    std::ranges::transform(sortable_sampler, std::begin(true_indices),
                           [](const auto &p) { return p.second; });
    std::inclusive_scan(std::begin(neg_sampler), std::end(neg_sampler),
                        std::begin(replacing_neg_sampler));
    neg_sampler_rand = std::uniform_real_distribution<float>(
        0, replacing_neg_sampler[num_cards - 1]);
  }

  CubeGenerator &enter() & {
    py::gil_scoped_release release;
    worker_thread = std::jthread([this](std::stop_token st) {
      this->worker_func(st, pcg32(this->initial_seed, 0));
    });
  ...
  void queue_new_epoch() {
    ...
    for (size_t i = 0; i < full_batches; i++) {
      tasks.emplace_back(indices.begin() + i * batch_size,
                         indices.begin() + (i + 1) * batch_size);
    }
    task_queue.enqueue_bulk(task_producer,
                            std::make_move_iterator(tasks.begin()), length);
  }
    ...
    std::vector<CubeData> *batched;
    {
      ...
      if (!processed_queue.wait_dequeue_timed(processed_consumer,
        ...
        while (!processed_queue.wait_dequeue_timed(processed_consumer,
          ...
          std::cout << "Waiting on a cube sample, without queueing."
                    << std::endl;
        }
      }
      if (processed_queue.size_approx() < 256)
        queue_new_epoch();
      batched = processed_value.release();
    }
    py::capsule free_when_done{
        batched, [](void *ptr) {
          delete reinterpret_cast<std::vector<CubeData> *>(ptr);
        }};
    CubeData &first_sample = batched->front();
    return {
        py::array_t<std::uint16_t>{noisy_cube_shape(), noisy_cube_strides,
                                   first_sample.noisy_cube.data(),
                                   free_when_done},
        py::array_t<std::uint16_t>{true_cube_shape(), true_cube_strides,
                                   first_sample.true_cube.data(),
                                   free_when_done},
    };
  }
  ...
  CubeData process_cube(const std::size_t index, pcg32 &rng) {
    ...
    const auto &row =
        reinterpret_cast<const CubeCards *>(cube_mmap.data())[index];
    for (const auto idx : row) {
      if (idx == 0)
        break;
      if (idx > num_cards) {
        std::cout << "Cube " << index << " has invalid index: " << idx
                  << std::endl;
        continue;
      }
      x1[idx - 1] = 1.f;
    ...
    auto to_include = sample_no_replacement(to_flip, neg_sampler * x1_inverse,
                                            true_indices, rng);
    std::valarray<float> to_exclude_sampler(0.0, num_cards);
    to_exclude_sampler[to_exclude] = neg_sampler[to_exclude];
    constexpr float y_to_exclude_perc = 0.0;
    auto y_to_exclude = sample_no_replacement(
        static_cast<std::size_t>(to_flip * y_to_exclude_perc),
        to_exclude_sampler, true_indices, rng);
  ...
  void worker_func(std::stop_token st, pcg32 rng) {
    ...
    while (!st.stop_requested()) {
      ...
      if (task_queue.wait_dequeue_timed(consume_token, task, 100'000)) {
        std::unique_ptr<std::vector<CubeData>> batch =
            std::make_unique<std::vector<CubeData>>();
...
private:
  constexpr std::array<std::size_t, 2> noisy_cube_shape() {
    return {batch_size, MAX_CUBE_SIZE};
  }
  constexpr std::array<std::size_t, 2> true_cube_shape() {
    return {batch_size, MAX_CUBE_SIZE};
  }

  static constexpr std::array<std::size_t, 2> noisy_cube_strides{
      sizeof(CubeData), sizeof(std::uint16_t)};
  static constexpr std::array<std::size_t, 2> true_cube_strides{
      sizeof(CubeData), sizeof(std::uint16_t)};
};
...
PYBIND11_MODULE(recommender_generator, m) {
  ...
  py::class_<CubeGenerator>(m, "RecommenderGenerator")
      .def(py::init<std::string, std::size_t, std::size_t, std::size_t, float,
                    float>())
