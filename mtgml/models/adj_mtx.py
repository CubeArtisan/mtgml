...
class AdjMtxReconstructor(ConfigurableLayer, tf.keras.Model):
    ...
    def get_properties(cls, hyper_config, input_shapes=None):
        ...
        # final_dims = hyper_config.get_int('embed_dims', default=256, min=8, max=1024, help='The number of dimensions for similarity comparisons')
        final_dims = input_shapes[2][-1] if input_shapes is not None else 8
        return {
            "num_cards": num_cards,
            'transform_single_card': hyper_config.get_sublayer('TransformSingleCard', sub_layer_type=MLP,
