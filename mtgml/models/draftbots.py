...
class DraftBot(ConfigurableLayer, tf.keras.Model):
    ...
    def get_properties(cls, hyper_config, input_shapes=None):
        ...
        return {
            'card_weights': hyper_config.get_list('card_weights', default=[0 for _ in range(num_cards)],
                                                  help=''),
    ...
    def call(self, inputs, training=False):
        ...
        if len(inputs) > 10:
            ...
            shifts = tf.reduce_sum(shifts, axis=-1)
            ...
            scores = (tf.cast(scores, dtype=loss_dtype, name='cast_scores') + tf.constant(LARGE_INT, dtype=loss_dtype)) * mask
            neg_scores = tf.math.negative(scores)
            pos_scores = scores
            ...
            return triplet_loss_weighted + log_loss_weighted
        return scores
