...
class DraftBot(ConfigurableLayer, tf.keras.Model):
    ...
    def call(self, inputs, training=False):
        ...
        pool = tf.ensure_shape(tf.cast(inputs[1], dtype=tf.int32, name='pool'), (None, None))
        seen_packs = tf.ensure_shape(tf.cast(inputs[2], dtype=tf.int32, name='seen_packs'), (None, None, None))
        seen_coords = tf.ensure_shape(tf.cast(inputs[3], dtype=tf.int32, name='seen_coords'), (None, None, 4, 2))
        seen_coord_weights = tf.ensure_shape(tf.cast(inputs[4], dtype=self.compute_dtype, name='seen_coord_weights'), (None, None, 4))
        ...
        pack_mask = tf.cast(tf.reduce_any(seen_packs > 0, axis=-1), dtype=self.compute_dtype, name='pack_mask')
        ...
        if len(inputs) > 6:
            ...
            loss = tf.reduce_mean(tf.cast(triplet_loss_weighted + log_loss_weighted + tf.reduce_mean(pool_variance * self.pool_variance_weight + seen_variance * self.seen_variance_weight
                                 + rating_variance * self.rating_variance_weight - score_variance * 1e-05), dtype=self.compute_dtype))
            return loss
