...
def should_skip_prop(structure, prop):
    ...
    if isinstance(structure, tf.keras.layers.Layer) and (prop == "state_updates" or prop == "updates"):
        return True
    if (
        hasattr(structure, "__class__")
        and hasattr(structure.__class__, prop)
...
def gen_leaves_with_path(structure, should_recurse=is_nested, prefix=(), visited=None):
    ...
    if should_recurse(structure, prefix) and struct_id not in visited:
        ...
        for key, child in gen_children_with_keys(structure):
            empty = False
            yield from gen_leaves_with_path(
                child, should_recurse=should_recurse, prefix=(*prefix, key), visited=visited
            )
        if empty:
...
def is_nested_or_module(structure, path):
    return (len(path) == 0 or not isinstance(path[-1], str) or path[-1] not in no_no_names) and (
        is_nested(structure, path) or isinstance(structure, tf.Module)
    )
...
if __name__ == "__main__":
    ...
    print(max(original_to_new_index))
    print(len(INT_TO_CARD))
    recommender_train_generator = RecommenderGenerator(
        "data/train_cubes.bin", len(INT_TO_CARD) - 1, cube_batch_size, seed, noise_mean, noise_std
    )
    deck_adj_mtx_generator = PickAdjMtxGenerator(
        "data/train_picks.bin", len(INT_TO_CARD) - 1, adj_mtx_batch_size, seed * 29
    )
    ...
    def combined_gen():
        for i in trange(32):
            ...
            yield (
                "call_draftbots",
                dict(
                    basics=basics, pool=pool, seen=seen, seen_coords=seen_coords, seen_coord_weights=seen_coord_weights
                ),
            )
    ...
    with draftbot_train_generator, recommender_train_generator:
        ...
        draftbot_result = call_draftbots(
            basics=basics, pool=pool, seen=seen, seen_coords=seen_coords, seen_coord_weights=seen_coord_weights
        )
