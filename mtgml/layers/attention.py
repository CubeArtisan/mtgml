...
def process_q_chunk(query, key_chunks, value_chunks, kv_partitions):
    ...
    with tf.name_scope("ProccessQChunk"):
        chunk_results = tf.map_fn(
            pass_along_query,
            (key_chunks, value_chunks),
            swap_memory=True,
            parallel_iterations=kv_partitions,
            infer_shape=False,
            fn_output_signature=(
                tf.TensorSpec((*query.shape[:-1], value_chunks.shape[-1]), dtype=query.dtype),
...
def calculate_attention(query, key, value, query_chunk_size, kv_chunk_size, name=None):
    with tf.name_scope(name or "MultiHeadAttention") as scope:
        ...
        chunk_results = tf.map_fn(
            pass_along_chunks, query_chunks, swap_memory=True, parallel_iterations=len(q_partitions)
        )
...
class MultiHeadAttention(ConfigurableLayer):
    ...
    def get_properties(cls, hyper_config, input_shapes=None):
        return {
            ...
            "value_dims": hyper_config.get_int(
                "value_dims", min=1, max=64, default=16, help="Size of the attention head for value."
            ),
            "output_dims": hyper_config.get_int(
                "output_dims", min=8, max=512, default=64, help="The number of output dimensions from this layer."
            ),
    ...
    def build(self, input_shapes):
        ...
        self.query_matrix = self.add_weight(
            "query_matrix", shape=(input_shapes[0][-1], self.num_heads * self.key_dims), trainable=True
        )
        self.key_matrix = self.add_weight(
            "key_matrix", shape=(input_shapes[1][-1], self.num_heads * self.key_dims), trainable=True
        )
        self.value_matrix = self.add_weight(
            "value_matrix", shape=(input_shapes[2][-1], self.num_heads * self.value_dims), trainable=True
        )
        self.output_matrix = self.add_weight(
            "output_matrix", shape=(self.num_heads * self.value_dims, self.output_dims), trainable=True
        )
    ...
    def call(self, inputs, training=False):
        ...
        query = tf.reshape(
            query @ self.query_matrix, (-1, self.query_sequence_length, self.num_heads, self.key_dims)
        ) / tf.constant(self.key_dims, dtype=self.compute_dtype)
        ...
        attended = calculate_attention(
            query, key, value, self.query_chunk_size, self.kv_chunk_size, name="DistributedAttention"
        )
        return (
            tf.reshape(attended, (-1, self.key_sequence_length, self.value_dims * self.num_heads)) @ self.output_matrix
        )
...
class MultiHeadAttentionBlock(ConfigurableLayer):
    ...
    def get_properties(cls, hyper_config, input_shapes=None):
        ...
        return {
            ...
            "feed_forward": hyper_config.get_sublayer(
                "FeedForward",
...
class InducedSetAttentionBlock(ConfigurableLayer):
    ...
    def get_properties(cls, hyper_config, input_shapes=None):
        return {
            ...
            "attention_h": hyper_config.get_sublayer(
                "AttentionForH",
                sub_layer_type=WMultiHeadAttention,
                help="The attention block that uses the inducing vectors",
            ),
            "attention_final": hyper_config.get_sublayer(
                "FinalAttention",
                sub_layer_type=WMultiHeadAttention,
                help="The attention block that recombines the input.",
            ),
            "num_inducing_vectors": hyper_config.get_int(
                "num_inducing_vectors",
                min=8,
                max=128,
                default=8,
                step=8,
    ...
    def build(self, input_shapes):
        ...
        self.inducing_vectors = self.add_weight(
            "inducing_vectors",
            shape=(self.num_inducing_vectors, input_shapes[-1]),  # shape=self.inducing_shape,
            trainable=True,
        )
...
class InducedSetAttentionBlockStack(ConfigurableLayer):
    @classmethod
    def get_properties(cls, hyper_config, input_shapes=None):
        num_hidden = hyper_config.get_int(
            "num_hidden", min=0, max=8, default=0, help="The number of hidden layers in the MLP."
        )
        props = {
            "hiddens": tuple(
                hyper_config.get_sublayer(
                    f"Hidden_{i}",
                    sub_layer_type=InducedSetAttentionBlock,
                    seed_mod=i + 1,
                    help=f'The {i+1}{"th" if i > 1 else "nd" if i == 1 else "st"} hidden layer.',
                )
                for i in range(num_hidden)
            ),
            "final": hyper_config.get_sublayer(
                "Final", sub_layer_type=InducedSetAttentionBlock, seed_mod=19, help="The last dense layer in the MLP."
            ),
        }
        if num_hidden > 0:
            props["dropout"] = hyper_config.get_sublayer(
                f"Dropout",
...
class PoolingByMultiHeadAttention(ConfigurableLayer):
    ...
    def get_properties(cls, hyper_config, input_shapes=None):
        return {
            "out_set_size": hyper_config.get_int(
                "out_set_size", min=1, max=128, default=None, help="The number of vectors in the output set."
            ),
            "attention": hyper_config.get_sublayer(
                "Attention",
                sub_layer_type=WMultiHeadAttention,
    ...
    def build(self, input_shapes):
        ...
        # self.inducing_shape[-1] = input_shapes[-1]
        self.seed_vectors = self.add_weight("seed_vectors", shape=(self.out_set_size, input_shapes[-1]), trainable=True)
    ...
    def call(self, inputs, training=False, mask=None):
        ...
        pooled = self.attention(
            self.seed_vectors + tf.zeros(shape, dtype=inputs.dtype), inputs, attention_mask=mask, training=training
        )
