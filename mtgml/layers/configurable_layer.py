                        ...
                        def calculate_tensor():
                            if name in int_metrics:
                                ...
                                labels = tf.range(
                                    buckets[0, 0],
                                    buckets[0, 0]
                                    + repeats * tf.cast(tf.shape(counts)[0], tf.float32, name="num_counts") * step_size,
                                    step_size,
                                    dtype=tf.float32,
                                    name=f"{name}_labels",
                                )
                                return tf.stack(
                                    [
                                        labels - step_size / 2,
                                        labels + step_size / 2,
                                        tf.repeat(counts, axis=0, repeats=repeats, name=f"{name}_counts_repeated"),
                                    ],
                                    axis=1,
                                ...
                                return tf.concat(
                                    [tf.transpose(cur_buckets), tf.expand_dims(counts, 1)],
                                    axis=1,
                                    name=f"{name}_histogram_data",
                                )
