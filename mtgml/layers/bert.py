...
class Transformer(ConfigurableLayer):
    ...
    def call(self, inputs, training=False, mask=None):
        if len(inputs) == 1:
            tokens = inputs
            attended = self.attention(tokens, tokens, training=training, mask=mask)
        else:
            tokens, attention_mask = inputs
            attended = self.attention(tokens, tokens, attention_mask=attention_mask, training=training, mask=mask)
