...
class MLP(ConfigurableLayer):
    ...
    def get_properties(cls, hyper_config, input_shapes=None):
        ...
        props = {
            'hiddens': [hyper_config.get_sublayer(f'Hidden_{i}', layer_type=Dense, seed_mod=i + 1,
                                                  help=f'The {i+1}{"th" if i > 1 else "nd" if i == 1 else "st"} hidden layer.'),
                        for i in range(num_hidden)]
            'final': hyper_config.get_sublayer('Final', layer_type=Dense, seed_mod=19,
                                               help='The last dense layer in the MLP.'),
        }
        if num_hidden > 0:
            props['dropout'] = hyper_config.get_sublayer(f'Dropout', layer_type=Dropout, seed_mod=11,
                                                         help='The dropout applied after each hidden layer.')
        return props

    def call(self, inputs, training=False, mask=mask):
