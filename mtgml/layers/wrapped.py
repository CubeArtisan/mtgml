...
class WMultiHeadAttention(WrappedLayer):
    ...
    def get_properties(cls, hyper_config, input_shapes=None):
        return {
            ...
            "dropout": hyper_config.get_float(
                "dropout", min=0, max=0.99, step=0.01, default=0.25, help="The percent of values to get dropped out"
            ),
    ...
    def build(self, input_shapes):
        ...
        self.wrapped_layer = tf.keras.layers.MultiHeadAttention(
            num_heads=self.num_heads,
            key_dim=self.key_dims,
            value_dim=self.value_dims,
            dropout=self.dropout,
            use_bias=self.use_bias,
            output_shape=(self.output_dims,),
...
class WDense(WrappedLayer):
    ...
    def get_properties(cls, hyper_config, input_shapes=None):
        return {
            "dims": hyper_config.get_int(
                "dims", min=8, max=512, default=32, help="The number of dimensions in the output of this layer."
            ),
    ...
    def build(self, input_shapes):
        ...
        self.wrapped_layer = tf.keras.layers.Dense(
            self.dims,
...
class WDropout(WrappedLayer):
    ...
    def build(self, input_shapes):
        ...
        self.wrapped_layer = tf.keras.layers.Dropout(
            rate=self.rate, noise_shape=self.noise_shape, seed=self.seed, name=self.name
        )
