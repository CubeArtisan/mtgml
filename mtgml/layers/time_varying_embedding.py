...
class TimeVaryingEmbedding(ConfigurableLayer):
    ...
    def get_properties(cls, hyper_config: HyperConfig, input_shapes=None):
        time_shape = hyper_config.get_list("time_shape", default=None, help="The dimensions of the time space.")
        if not time_shape:
            raise NotImplementedError("You must supply a time shape.")
        return {
            "time_shape": time_shape,
            "dims": hyper_config.get_int(
                "dims", min=8, max=256, default=32, help="The number of dimensions the cards should be embedded into."
            ),
    ...
    def build(self, input_shapes):
        ...
        if self.dims == 3:
            initializer = tf.constant_initializer(
                np.zeros(shape, dtype=np.float32) + np.log(np.exp([0.1, 0.01, 10.0]) - 1.0)
            )
        else:
    ...
    def call(self, inputs, training=False, mask=None):
        if isinstance(inputs, (list, tuple)):
            ...
            component_embedding_values = self.activation(
