    ...
    if isinstance(draft, dict):
        ...
        if "picks" in draft:
            ...
            for pick in draft["picks"]:
                ...
                coords, coord_weights = interpolate(
                    pick["pickNum"], pick["numPicks"], pick["packNum"], pick["numPacks"]
                )
                ...
                card = cards_in_pack[0]
                if "pickedIdx" in pick:
                    picked.append(card)
                    last_idx = -1
                else:
                    last_idx = len(picked)
                if any(x <= 0 or x > max_index for x in cards_in_pack) or card <= 0 or card > max_index:
                    break
            ...
            if len(picked) > 0:
                yield (basics, picked[:last_idx], seen, seen_coords, seen_coord_weights, trashed)
...
def load_all_drafts(pool, *args):
    for drafts_dir, picks_gen in zip(args, (picks_from_draft,)):
        ...
        def load_drafts_file(drafts_file):
            ...
            try:
                with open(drafts_file, "rb") as fp:
                    ...
                    def gen():
                        for draft in tqdm(
                            drafts, leave=False, dynamic_ncols=True, unit="draft", unit_scale=1, smoothing=0.001
                        ):
        ...
        for draft_dir in drafts_dir.split(";"):
            ...
            yield from tqdm(
                (load_drafts_file(file) for file in files),
                leave=False,
                dynamic_ncols=True,
                unit="file",
                unit_scale=1,
...
DEVOTION = np.array(
    ...
    + [
        calculate_devotion(c["parsed_cost"], c["cmc"], c["color_identity"], c["type"])
        for c in (int_to_card[i - 1] for i in new_index_to_original[1:])
    ]
)
COLOR_IDENTITY = np.array(
    [np.zeros((5,), dtype=np.float32)]
    + [colors_to_mask(card["color_identity"]) for card in (int_to_card[i - 1] for i in new_index_to_original[1:])]
)
...
PREFIX = struct.Struct(
    f"{MAX_BASICS}H{MAX_PICKED}H{MAX_SEEN_PACKS * MAX_CARDS_IN_PACK}H{MAX_SEEN_PACKS * 4 * 2}B{MAX_SEEN_PACKS * 4}f{MAX_SEEN_PACKS * MAX_CARDS_IN_PACK}f{MAX_SEEN_PACKS}B"
)
...
def write_pick(pick, riskiness, output_file):
    ...
    prefix = PREFIX.pack(
        *pad(basics, MAX_BASICS),
        *pad(picked, MAX_PICKED),
        *seen,
        *seen_coords,
        *seen_coord_weights,
        *riskiness,
        *pad(trashed, MAX_PICKED),
    )
...
def read_pick(input_file, offset):
    ...
    parsed = parsed[4:]
    trashed = parsed[0]
    return cards_in_pack, basics, picked, seen, seen_coords, seen_coord_weights, coords, coord_weights, trashed


def dump_picks(picks, input_file, dest_folder):
    context_count = len(picks)
    cards_in_pack = np.memmap(
        dest_folder / "cards_in_pack.npy", dtype=np.int16, mode="w+", shape=(context_count, MAX_CARDS_IN_PACK)
    )
    ...
    seen = np.memmap(
        dest_folder / "seen.npy", dtype=np.int16, mode="w+", shape=(context_count, MAX_SEEN_PACKS, MAX_CARDS_IN_PACK)
    )
    seen_coords = np.memmap(
        dest_folder / "seen_coords.npy", dtype=np.int8, mode="w+", shape=(context_count, MAX_SEEN_PACKS, 4, 2)
    )
    seen_coord_weights = np.memmap(
        dest_folder / "seen_coord_weights.npy", dtype=np.float32, mode="w+", shape=(context_count, MAX_SEEN_PACKS, 4)
    )
    ...
    for context_idx, offset in enumerate(
        tqdm(picks, leave=False, dynamic_ncols=True, unit="picks", unit_scale=1, smoothing=0.001)
    ):
    ...
    for name, arr in (
        ...
        ("seen", seen),
        ("seen_coords", coords),
        ("seen_coord_weights", coord_weights),
        ("coords", coords),
...
if __name__ == "__main__":
    ...
    ) as evaluation_file, multiprocessing.pool.ThreadPool(processes=1) as read_pool, multiprocessing.pool.ThreadPool(
        processes=1
    ) as write_pool:
