...
if __name__ == "__main__":
    ...
    card_embeddings = np.load(output_dir + 'card_embeddings.npy') / 10_000
    card_embeddings = np.concatenate((np.zeros((1, card_embeddings.shape[1]), dtype=card_embeddings.dtype), card_embeddings))
    config_path = Path('ml_files')/args.name/'draftbot_hyper_config.yaml'
    data = {}
    ...
    with strategy.scope():
        model = hyper_config.build(name='DraftBot', dynamic=False)
    if model is None:
        sys.exit(1)
    ...
    with draftbot_train_generator, draftbot_validation_generator, strategy.scope():
        train_generator = \
            SplitGenerator(draftbot_train_generator,
        ...
        # validation_generator = draftbot_validation_generator
        with open(config_path, 'w') as config_file:
            yaml.dump(hyper_config.get_config(), config_file)
        with open(log_dir + '/draftbot_hyper_config.yaml', 'w') as config_file:
            yaml.dump(hyper_config.get_config(), config_file)
        callbacks = []
        if not args.debug:
            mcp_callback = tf.keras.callbacks.ModelCheckpoint(
                filepath=output_dir + 'draftbot_model',
                verbose=True,
        ...
        pick_train_example = train_generator[0][0][:8]
