...
def get_draft_scores(drafter_state, model, card_to_int, tracer, original_to_new_index):
    ...
    with tracer.start_as_current_span('call_draftbots'):
        ...
        result = call_draftbots(basics=basics, pool=pool, seen=seen_packs, seen_coords=seen_coords, seen_coord_weights=seen_weights)
    ...
    oracles = [[dict(metadata) | { 'weight': float(weight), 'score': float(score) }
                ...
                in zip(METADATA, oracle_weights, card_scores)]
