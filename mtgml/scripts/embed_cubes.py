                ...
                for cube in tqdm(cubes, leave=False, dynamic_ncols=True, unit='cube', unit_scale=1,
                    ...
                    try:
                        if MAX_CUBE_SIZE >= len(cube['cards']) >= 120 and all(x in name_to_int for x in cube['cards']):
                            num_cubes += 1
                            yield (pad(tuple(name_to_int[x] + 1 for x in cube['cards']), MAX_CUBE_SIZE),
                                   cube['name'], cube['id'])
                    except:
                        continue
...
if __name__ == "__main__":
    ...
    with open('data/cube_embeddings.csv', 'w') as embed_handle, open('data/cube_data.csv', 'w') as data_handle:
        ...
        for cards, name, id in cubes:
            embedding = model.cube_recommender((np.array([cards]), model.embed_cards.embeddings),
                                               training=False)[1].numpy()[0]
